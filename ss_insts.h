...
#define SS_GARBAGE_GENERAL(output_port, num_elem, elem_size) \
  do { \
    int _elem_pow = 31 - __builtin_clz((int32_t) elem_size); \
    _elem_pow <<= 8; \
    int _port = (output_port) | (1 << 7); \
    int imm = (_port | _elem_pow) << 1; \
    __asm__ __volatile__("ss_wr_dma %0, %1, %2" : : "r"(0), "r"(num_elem), "i"(imm)); \
  } while (false) 
...
/* DMA */
#define SS_DMA_RD_INNER(addr, n, elem_size, port) \
  do { \
    int _elem_pow = 31 - __builtin_clz((int32_t) elem_size); \
    __asm__ __volatile__("ss_dma_rd %0, %1, %2" : : "r"(addr), "r"(n), "i"((port | _elem_pow << 8) << 1)); \
  } while (false)

#define SS_DMA_RD_OUTER(stride, n, stretch) \
  __asm__ __volatile__("ss_dma_rd %0, %1, %2" : : "r"(stride), "r"(n), "i"((stretch) << 1 | 1))

#define SS_DMA_WR_INNER(addr, n, elem_size, port) \
  do { \
    int _elem_pow = 31 - __builtin_clz((int32_t) elem_size); \
    __asm__ __volatile__("ss_wr_dma %0, %1, %2" : : "r"(addr), "r"(n), "i"((port | _elem_pow << 8) << 1)); \
  } while (false)
...
/* Scratchpad */
#define SS_SCR_RD_INNER(addr, n, elem_size, port) \
  do { \
    int _elem_pow = 31 - __builtin_clz((int32_t) elem_size); \
    __asm__ __volatile__("ss_scr_rd %0, %1, %2" : : "r"(addr), "r"(n), "i"((port | _elem_pow << 8) << 1)); \
  } while (false)

#define SS_SCR_RD_OUTER(stride, n, stretch) \
  __asm__ __volatile__("ss_scr_rd %0, %1, %2" : : "r"(stride), "r"(n), "i"((stretch) << 1 | 1))

#define SS_SCR_WR_INNER(addr, n, elem_size, port) \
  do { \
    int _elem_pow = 31 - __builtin_clz((int32_t) elem_size); \
    __asm__ __volatile__("ss_wr_scr %0, %1, %2" : : "r"(addr), "r"(n), "i"((port | _elem_pow << 8) << 1)); \
  } while (false)
...
#define SS_SCR_PORT_STREAM_STRETCH(scr_addr,stride,acc_size,stretch,n_strides, port) \
  do { \
    if (acc_size > 0) { \
      int _size = 1; \
      int _stride = (stride); \
      int _inner_cnt = (acc_size) / _size; \
      int _outer_cnt = n_strides; \
      int _stretch = (stretch) / _size; \
      SS_SCR_RD_OUTER(_stride, _outer_cnt, _stretch); \
      SS_SCR_RD_INNER(scr_addr, _inner_cnt, _size, port);  \
    } else { \
      int _addr = scr_addr + acc_size; \
      int _size = 1; \
      int _stride = (stride); \
      int _inner_cnt = -acc_size; \
      int _outer_cnt = n_strides; \
      int _stretch = (stretch) / _size; \
      SS_SCR_RD_OUTER(_stride, _outer_cnt, _stretch); \
      SS_SCR_RD_INNER(_addr, _inner_cnt, _size, port);  \
    } \
  } while (false)
...
#define SS_DMA_READ_STRETCH(mem_addr, stride, acc_size, stretch, n_strides, port ) \
  do { \
    ...
    int _outer_cnt = n_strides; \
    int _stretch = (stretch) / _size; \
    SS_DMA_RD_OUTER(_stride, _outer_cnt, _stretch); \
    SS_DMA_RD_INNER(mem_addr, _inner_cnt, _size, port); \
  } while (false)
...
#define SS_DMA_WRITE(output_port, stride, access_size, num_strides, mem_addr) \
  do { \
    int _size = 1; \
    int _stride = (stride); \
    int _inner_cnt = (access_size) / _size; \
    int _outer_cnt = num_strides; \
    SS_DMA_WR_OUTER(_stride, _outer_cnt, 0); \
    SS_DMA_WR_INNER(mem_addr, _inner_cnt, _size, output_port); \
  } while (false)
...
#define SS_SCR_WRITE(output_port, num_bytes, scr_addr) \
  do { \
    SS_SCR_WR_INNER(scr_addr, num_bytes, 1, output_port); \
  } while (false)
